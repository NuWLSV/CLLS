...
void ISDist::build_instance(char *filename)
{
    ...
    while (getline(infile, line))
    {
        ...
        if (top_clause_weight == org_clause_weight[c])
        {
            if(clause_lit_count[c] == 1)
            {
                fixed[clause_lit[c][0].var_num] = 1;
            }
...
void ISDist::allocate_memory()
{
    ...
    Max_Sample_Num = 30;
    sample_num = 0;
    sample_unsat_clauses = new int[malloc_clause_length];
    clause_in_sample = new int[num_clauses];
    for(int i = 0;i < num_clauses;i++){
        clause_in_sample[i] = 0;
    }
    sample_clause_weight = new double[malloc_clause_length];
    #endif
    fixed = new int[malloc_var_length];
