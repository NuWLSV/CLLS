...
#define DOUBEL_MIN numeric_limits<double>::lowest()
...
class ISDist
{
  private:
	...
	int Max_Sample_Num;
    int sample_num;
    int* sample_unsat_clauses;
    int* clause_in_sample;
	double* sample_clause_weight;
	...
	int  pick_var(SOL_TANK* tank);
	int  pick_var();
	bool clauses_update(SOL_TANK* tank, int* unsat_stack, int unsat_stack_fill_pointer);
  public:
	ISDist();
	void settings();
	...
	void print_unsat_hard_clauses();
};

void ISDist::print_unsat_hard_clauses(){
	for(int i = 0;i < hard_unsat_nb;i++){
        printf("%d ", hardunsat_stack[i]);
        if(i % 10 == 9)printf("\n");
    }
}
