...
class Decimation
{
  public:
    ...
    void random_propagation(SOL_TANK* tank);
    void unit_prosess();
    void unit_prosess(SOL_TANK* tank);
...
void Decimation::random_propagation(SOL_TANK* tank)
{
    int v, sense;
    v = unassigned_var[rand() % unassigned_var_count];
    sense = tank -> vote(v);
...
void Decimation::unit_prosess(SOL_TANK* tank)
{   
    while(hunit_beg_pointer != hunit_end_pointer){
        int v, sense;
        v = hunit_clause_queue[hunit_beg_pointer].var_num;
        sense = hunit_clause_queue[hunit_beg_pointer].sense; // literal
        hunit_beg_pointer++;
        if(!tank -> fixed[v])tank -> fixed[v] = 1;
        assign(v, sense);
    }

    if(!tank->select_x(fix)){
        while (unassigned_var_count > 0)
        {
            if (hunit_beg_pointer != hunit_end_pointer)
            {
